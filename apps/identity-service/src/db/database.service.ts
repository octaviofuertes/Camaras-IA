...
export class DatabaseService implements OnModuleInit, OnModuleDestroy {
  ...
  private pool!: Pool;
  /** Sólo para el login (ver asAdmin). Se crea perezosamente. */
  ...
  /**
   * Conexión administrativa (superusuario), EXCLUSIVA para autenticación.
   *
   * El login es previo al contexto de tenant: hay que encontrar al usuario por
   * email para recién ahí saber a qué organización pertenece, y la RLS del rol
   * de aplicación lo impediría. Todo lo demás del sistema usa `withTenant`.
   *
   * Regla: acá SÓLO se consultan `users`, `user_roles` y `role_permissions`,
   * y siempre filtrando por un identificador concreto.
   */
  async asAdmin<T>(fn: (client: PoolClient) => Promise<T>): Promise<T> {
    if (!this.adminPool) {
      const cs = process.env.DATABASE_URL_ADMIN;
      if (!cs) throw new Error('DATABASE_URL_ADMIN no está definida (necesaria para el login)');
      this.adminPool = new Pool({ connectionString: cs, max: 4 });
    }
